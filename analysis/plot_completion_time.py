...
df = pd.read_csv('data/completed_time_merged.csv')
print(df.columns)
df_all = df.copy()
nall = str(df_all['user'].count())
df_all['category'] = 'All (' + nall + ')'
df = pd.concat([df_all, df])
df = df.rename(columns={'completion_time': 'Completion Time'})
...
ax = sns.violinplot(x="category", y="Completion Time", hue="uses_plugin",
...
plt.savefig('figures/completion_time_violin.pdf',bbox_inches='tight')
